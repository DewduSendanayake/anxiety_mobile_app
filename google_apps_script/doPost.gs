...
function doPost(e) {
  ...
  if (entries.length > MAX_ROWS_PER_POST)
    return jsonErr(
      "Batch too large. Max " + MAX_ROWS_PER_POST + " per request.",
    );
  ...
  // 6. Process each user independently
  var totalWritten = 0;
  var errors = [];
...
function categorizeApp(pkg) {
  ...
  if (
    /whatsapp|telegram|signal|viber|messenger|instagram|snapchat|tiktok|facebook|twitter|linkedin/.test(
      p,
    )
  )
    return "Social_Media";
  if (/chrome|firefox|brave|opera|samsung.*internet|browser/.test(p))
    return "Browser";
  if (/youtube|netflix|spotify|prime.*video|disney|media/.test(p))
    return "Entertainment";
  if (/gmail|outlook|mail|email/.test(p)) return "Email";
  if (/maps|waze|uber|grab|ola|navigation|gps/.test(p))
  ...
  if (/health|fitness|medic|hospital|therapy|mental|anxiety|doctor/.test(p))
  ...
  entries.forEach(function (item) {
    ...
    try {
      ...
      if (family === "Raw") {
        ...
        var period =
          ema.period ||
          String(item.dataType || "").replace("EMA_Rating_", "") ||
          "unknown";
        row = [
          ts,
          date,
          ...
          userId,
          capitalize(period),
          ema.rating !== undefined ? ema.rating : "",
          ema.context || "",
          valStr,
        ];
      } else if (family === "GAD7") {
        ...
        row = [
          ...
          date,
          userId,
          gad.total_score !== undefined ? gad.total_score : "",
          gad.severity || "",
          ans[0] || "",
          ans[1] || "",
          ans[2] || "",
...
function writeRows(tab, rows) {
  try {
    tab
      .getRange(tab.getLastRow() + 1, 1, rows.length, rows[0].length)
      .setValues(rows);
  } catch (_) {
    rows.forEach(function (row) {
      try {
        ...
        console.error("[appendRow failed] " + e2);
      }
    });
  }
}
...
function getOrCreateSpreadsheet(userId, props) {
  ...
  if (ssId) {
    try {
      ...
      console.warn(
        "[getOrCreateSpreadsheet] stored ID invalid for " +
          userId +
          ", recreating",
      );
  ...
  if (folderId) {
    try {
      ...
      try {
        ...
        console.warn(
          "[root remove failed — file is in both root and folder] " + removeErr,
        );
      }
  ...
  emails.split(",").forEach(function (email) {
    ...
    if (email) {
      try {
        ...
        console.warn("[addViewer failed] " + email + ": " + e);
      }
    }
  });
  ...
  console.log(
    "[created] user=" + userId + " id=" + ss.getId() + " url=" + ss.getUrl(),
  );
...
function getOrCreateTab(ss, tabName, family) {
  ...
  if (!tab) {
    ...
    applyHeaders(tab, family);
  } else if (tab.getLastRow() === 0) {
    applyHeaders(tab, family);
  }
  return tab;
}
...
function initSummary(tab, userId) {
  var data = [
    ...
    [
      "Privacy Note",
      "GPS fuzzed ±1km | Call/SMS counts only | Apps categorised",
    ],
  ];
...
function updateSummary(ss, userId) {
  ...
  ss.getSheets().forEach(function (s) {
    var n = s.getName();
    if (n.indexOf("Raw_") === 0) {
      rawRows += Math.max(0, s.getLastRow() - 1);
      months.push(n.replace("Raw_", ""));
    }
  ...
  tab
    .getRange("B8")
    .setValue(demo ? (demo.getLastRow() > 1 ? "Yes" : "No") : "No");
...
function setupScript() {
  ...
  try {
    ...
    Logger.log("Restrict folder manually in Drive: " + e);
  }

  props.setProperty("DRIVE_FOLDER_ID", folder.getId());
  props.setProperty("AUTH_TOKEN", "d658907d174e6ae0a12a4b6eb66ad365");
  props.setProperty(
    "RESEARCHER_EMAILS",
...
function diagnoseSetup() {
  ...
  Logger.log(
    "AUTH_TOKEN set:    " +
      (props.getProperty("AUTH_TOKEN") ? "YES ✅" : "NO ❌"),
  );
  Logger.log(
    "DRIVE_FOLDER_ID:   " +
      (props.getProperty("DRIVE_FOLDER_ID") || "NOT SET ❌"),
  );
  Logger.log(
    "RESEARCHER_EMAILS: " +
      (props.getProperty("RESEARCHER_EMAILS") || "NOT SET"),
  );

  var ssKeys = Object.keys(all).filter(function (k) {
    return k.indexOf("SS_") === 0;
  });
  ...
  if (folderId) {
    try {
      ...
      if (count === 0)
        Logger.log("  (folder is empty — run testDoPost to populate)");
    } catch (e) {
...
function testDoPost() {
  ...
  var entries = [
    ...
    {
      userId: "TEST_P001",
      dataType: "App_Usage_15m",
      value: JSON.stringify({
        "com.whatsapp": "120.0s",
        "com.android.chrome": "80.5s",
        "com.therapyapp.android": "30.0s",
      }),
      timestamp: now,
      token: token,
    },
    {
      userId: "TEST_P001",
      dataType: "EMA_Rating_morning",
      value: JSON.stringify({
        rating: 2,
        context: "Studying / Working",
        period: "morning",
      }),
      timestamp: now,
      token: token,
  ...
  if (parsed.status === "success" || parsed.status === "partial") {
    ...
    if (newId) {
        ...
        if (s.getName().indexOf("Raw_") === 0 && s.getLastRow() > 1) {
          ...
          for (var i = 1; i < data.length; i++) {
            if (data[i][4] === "Location") {
              ...
              Logger.log(
                "GPS fuzzed:   " +
                  (loc.lat !== 6.927123 ? "✅ YES" : "❌ NO — check fuzzGPS()"),
              );
            }
      ...
      if (folderId) {
        ...
        Logger.log(
          "In correct folder: " +
            (inFolder ? "✅ YES" : "❌ NO — check Drive root for stray file"),
        );
      }
    }
  } else {
...
function fixMisplacedSpreadsheets() {
  ...
  var folderId = props.getProperty("DRIVE_FOLDER_ID");
  if (!folderId) {
    Logger.log("No DRIVE_FOLDER_ID set.");
    return;
  }
...
function listAllParticipants() {
  ...
  for (var key in all) {
    ...
    try {
      ...
      var raw = 0;
      ss.getSheets().forEach(function (s) {
        if (s.getName().indexOf("Raw_") === 0)
          raw += Math.max(0, s.getLastRow() - 1);
      });
...
function rotateAuthToken(newToken) {
  if (!newToken || newToken.length < 16) {
    Logger.log("Token must be ≥16 chars.");
    return;
  }
  PropertiesService.getScriptProperties().setProperty("AUTH_TOKEN", newToken);
  Logger.log(
    "Rotated. Update _authToken in background_service_helper.dart and rebuild APK.",
  );
}
...
function deleteTestData() {
  ...
  for (var key in all) {
    ...
    try {
      DriveApp.getFileById(all[key]).setTrashed(true);
